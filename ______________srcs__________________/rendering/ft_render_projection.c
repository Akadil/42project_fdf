...
void	ft_draw_line(t_data *data, int p0[2], int p1[2], int colors[2])
{
	...
	{
		my_mlx_pixel_put(data, line.y0, line.x0, ft_color(p0[0], p1[0], line.x0, colors));
		line.e2 = 2 * line.err;
		if (line.e2 > -line.dy)
		{
			line.err -= line.dy;
	...
	my_mlx_pixel_put(data, line.y0, line.x0, ft_color(line.x0, p1[0], line.x0, colors));
}
...
void	ft_transform(t_data *data, int ret[2], int x, int y)
{
	...
	ret[0] = x * data->coef.grid * cos(alpha_rad);
	ret[0] -= data->mtrx.matrix[x][y] * data->coef.attitude * sin(beta_rad);
	ret[0] += data->coef.x_offset;
	ret[1] = x * data->coef.grid * sin(alpha_rad) * sin(beta_rad);
	ret[1] += y * data->coef.grid * cos(alpha_rad);
	ret[1] += data->mtrx.matrix[x][y] * data->coef.attitude * sin(alpha_rad)
		* cos(beta_rad);
	ret[1] += data->coef.y_offset;
}
